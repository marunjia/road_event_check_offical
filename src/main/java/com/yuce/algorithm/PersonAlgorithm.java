...
public class PersonAlgorithm {
    ...
    @Autowired
    GeneralAlgorithm generalAlgorithm;
    ...
    //定义接口请求访问地址
    private static final String PERSON_URL = "http://12.1.97.206:7860/detect";
    ...
    public boolean personDeal(OriginalAlarmRecord record) {
        ...
        if(checkAlarmResultMapper.getResultByKey(alarmId,imagePath,videoPath) != null) {
            log.info("行人算法已检过：id->{}, alarmId->{}, imagePath->{}, videoPath->{}", id, alarmId, imagePath, videoPath);
            return true;
        }
        ...
        Request request = new Request.Builder().url(PERSON_URL).post(body).build();

        // 执行同步请求
        try (Response response = client.newCall(request).execute()) {
            if (response == null || !response.isSuccessful()) {
                log.error("行人算法接口返回非成功状态: code={}, alarm_id->{}", response != null ? response.code() : "null",alarmId);
                return false;
            }

            // 关键：只读取一次响应体，并确保日志中不重复调用
            String responseBody = response.body() != null ? response.body().string() : null;
            log.info("调用行人算法服务:alarmId -> {}, 请求体内容 -> {}, 返回体 -> {}", alarmId, jsonBody, responseBody);
            ...
            for (int i = 0; i < resultArray.size(); i++) {
                ...
                if (dataArray.size() > 0) {
                    for (int j = 0; j < dataArray.size(); j++) {
                        ...
                        checkAlarmProcess.setReceivedTime(LocalDateTime.now());
            ...
            return checkAlarmProcessServiceImpl.saveBatch(checkProcessList);
        } catch (IOException e) {
            log.error("算法请求异常", e);
            return false;
        }
    ...
    public Map<String, Object> buildRequestData(OriginalAlarmRecord record) {
        ...
        List<FrameImageInfo> list = frameImageServiceImpl.getFrameListByKey(record.getId(),record.getImagePath(),record.getVideoPath());
        ...
        for (FrameImageInfo imageInfo : list) {
            ...
            // imageId规则:{alarmId}_{图片编号}_{原始告警记录tblId}_{告警日期}
            ...
            bboxColorMap.put("bbox_color", extractWindowAlgorithm.getExtractColor(record));
