...
public class VehicleAlgorithm {
    ...
    //定义接口请求访问地址
    private static final String VEHICLE_URL = "http://12.1.97.206:7860/detect";
    ...
    public boolean vehicleDeal(OriginalAlarmRecord record) {
        ...
        if(checkAlarmResultMapper.getResultByKey(alarmId,imagePath,videoPath) != null) {
            log.info("停驶算法已检过：id->{}, alarmId->{}, imagePath->{}, videoPath->{}", id, alarmId, imagePath, videoPath);
            return true;
        }
        ...
        Request request = new Request.Builder().url(VEHICLE_URL).post(body).build();

        // 执行同步请求
        try (Response response = client.newCall(request).execute()) {
            if (response == null || !response.isSuccessful()) {
                log.error("停驶算法接口返回非成功状态: code={}, alarm_id->{}", response != null ? response.code() : "null",alarmId);
                return false;
            }

            // 关键：只读取一次响应体，并确保日志中不重复调用
            String responseBody = response.body() != null ? response.body().string() : null;
            log.info("调用停驶算法服务:alarmId -> {}, 请求体内容 -> {}, 返回体 -> {}", alarmId, jsonBody, responseBody);
