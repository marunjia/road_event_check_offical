...
public class OriginalAlarmServiceImpl extends ServiceImpl<OriginalAlarmMapper, OriginalAlarmRecord> implements OriginalAlarmService {
    ...
    public OriginalAlarmRecord getRecordByKey(String alarmId, String imagePath, String videoPath) {
        ...
        if(imagePath == null) {
            wrapper.isNull("image_path");
        }else{
            wrapper.eq("image_path", imagePath);
        }

        if(imagePath == null) {
            wrapper.isNull("video_path");
        }else{
            wrapper.eq("video_path", videoPath);
        }
        return this.getOne(wrapper);
    }
     ...
     * @param alarmTime 时间区间范围
     * @return 匹配的告警记录列表，无匹配时返回空列表（非 null）
     */
    public List<OriginalAlarmRecord> getListRecnet10MinByTime(LocalDateTime alarmTime) {
        LambdaQueryWrapper<OriginalAlarmRecord> wrapper = new LambdaQueryWrapper<>();
        // 计算区间左边界：alarmTime往前推10分钟（含）
        LocalDateTime startDateTime = alarmTime.minusMinutes(10);

        // 核心：精准限定 [startDateTime, alarmTime) 区间
        wrapper.ge(OriginalAlarmRecord::getAlarmTime, startDateTime)  // >= 10分钟前的时间（左闭）
                .lt(OriginalAlarmRecord::getAlarmTime, alarmTime)       // < 基准时间（右开）
                .orderByDesc(OriginalAlarmRecord::getAlarmTime);        // 按告警时间降序，最新的排在前面
     ...
     */
    public boolean saveOrUpdateRecord(OriginalAlarmRecord record) {
        ...
        //查询记录是否存在
        OriginalAlarmRecord existingRecord = getRecordByKey(alarmId, imagePath, videoPath);
        ...
        if(existingRecord != null) {
            ...
            if(flag){
                log.info("告警记录已存在，更新成功 | tblId:{} | alarmId:{} | imagePath:{} | videoPath:{}", existingRecord.getTblId(), alarmId, imagePath, videoPath);
            }else{
                log.info("告警记录已存在，更新失败 | tblId:{} | alarmId:{} | imagePath:{} | videoPath:{}", existingRecord.getTblId(), alarmId, imagePath, videoPath);
            }
            return true;
        }else{
            //新增告警记录
     ...
     * @return 匹配的告警记录列表，无匹配时返回空列表（非 null）
     */
    public List<OriginalAlarmRecord> getListByTblIdListAndTimeAgo(List<String> relatedTblIdList, LocalDateTime alarmTime) {
        // 校验列表合法性：避免 in 空列表导致的 SQL 错误（如 WHERE tbl_id IN ()）
        Assert.notEmpty(relatedTblIdList, "tblId 列表不能为空且不能包含空元素");
        Assert.isTrue(relatedTblIdList.stream().noneMatch(StringUtils::isEmpty), "tblId 列表中不能包含空元素");
    ...
    public List<OriginalAlarmRecord> getListByTblIdListAndTimeAfter(List<String> relatedTblIdList, LocalDateTime alarmTime) {
        // 校验列表合法性：避免 in 空列表导致的 SQL 错误（如 WHERE tbl_id IN ()）
        Assert.notEmpty(relatedTblIdList, "tblId 列表不能为空且不能包含空元素");
        Assert.isTrue(relatedTblIdList.stream().noneMatch(StringUtils::isEmpty), "tblId 列表中不能包含空元素");
        ...
        wrapper.in(OriginalAlarmRecord::getTblId, relatedTblIdList)
                ...
                .orderByAsc(OriginalAlarmRecord::getAlarmTime);
        // 安全查询：即使无数据也返回空列表，避免上层处理 null
        return this.list(wrapper);
    }
     ...
     * @param startTime 开始日期（可选，格式 yyyy-MM-dd）
     * @param endTime 结束日期（可选，格式 yyyy-MM-dd）
     ...
     */
    public IPage<QueryResultCheckRecord> selectWithOriginaleField(String alarmId, String startTime, String endTime, String deviceName, String roadId, String directionDes, String eventType, Integer dealFlag, Integer checkFlag, Integer disposalAdvice, String adviceReason, String deviceId, int pageNo, int pageSize) {
        ...
        if (StringUtils.hasText(startTime)) {
            ...
            query.ge("o.alarm_time", startTimeFormat);
        }

        if (StringUtils.hasText(endTime)) {
            LocalDateTime endTimeFormat = LocalDateTime.parse(endTime, formatter);
            query.le("o.alarm_time", endTimeFormat);
        }
