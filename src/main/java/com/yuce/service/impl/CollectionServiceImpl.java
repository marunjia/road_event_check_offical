...
import com.yuce.service.CollectionService;
...
public class CollectionServiceImpl implements CollectionService {
     ...
     * @param alarmId
     */
    public void collectionDeal(String alarmId) {
        int checkFlag = checkAlarmResultServiceImpl.getByAlarmId(alarmId).getCheckFlag();//查询算法检测结果

        /**
         * 查询原始告警记录信息
         */
        OriginalAlarmRecord originalAlarmRecord = originalAlarmServiceImpl.getById(alarmId);
        String roadId = originalAlarmRecord.getRoadId();
        String deviceId = originalAlarmRecord.getDeviceId(); //获取设备id
        LocalDateTime alarmTime = originalAlarmRecord.getAlarmTime();
        String deviceName = getDeviceName(originalAlarmRecord.getContent());
        String eventType = originalAlarmRecord.getEventType();
        int milestone = originalAlarmRecord.getMilestone();

        AlarmCollection alarmCollection = alarmCollectionServiceImpl.existsCollection(deviceId, checkFlag);//根据deviceId、checkFlag判断告警集是否存在
        ...
        if (alarmCollection != null) {
            ...
            if (minutes > getIntervalDuration(checkFlag)) {
                ...
                insertAlarmCollection(roadId, deviceId, deviceName, eventType, checkFlag, alarmId, alarmTime, milestone);//开启新告警集并插入数据
            } else {
                List<String> list = new ArrayList<>(Arrays.asList(alarmCollection.getRelatedIdList().split(",")));//获取当前告警集关联事件列表
                if(!list.contains(alarmId)){
                    list.add(alarmId);
                    ...
                    alarmCollectionServiceImpl.saveOrUpdate(alarmCollection);
                }
            }
        } else {
            insertAlarmCollection(roadId, deviceId, deviceName, eventType, checkFlag, alarmId, alarmTime, milestone);
        }
    }
     ...
     * @param alarmTime
     */
    public void insertAlarmCollection(String roadId, String deviceId, String deviceName, String eventType, int checkFlag, String alarmId, LocalDateTime alarmTime, int milestone) {
    ...
    public String typeDefineDeal(List<String> ids) {
        ...
        List<OriginalAlarmRecord> originalList = originalAlarmServiceImpl.getListByIdList(ids);
        ...
        for (OriginalAlarmRecord record : originalList) {
            CheckAlarmProcess process = checkAlarmProcessServiceImpl.getIouTop1ByAlarmId(record.getId());//每个事件返回一个检测结果
    ...
    public int getCollectionAdvice(AlarmCollection alarmCollection) {
        ...
        List<OriginalAlarmRecord> originals = originalAlarmServiceImpl.getListByIdList(ids);//获取告警集关联的记录
        List<OriginalAlarmRecord> confirmed = originalAlarmServiceImpl.getListByDealFlag(ids,1);//获取被确定为事件的记录
        List<OriginalAlarmRecord> unConfirmed = originalAlarmServiceImpl.getUnConfirmList(ids);//获取未被确定为事件的记录
        List<CheckAlarmResult> results = checkAlarmResultServiceImpl.getByAlarmIdList(ids);
        ...
        if(confirmed.size() == 0){
            //遍历所有告警记录的处置建议
            for(OriginalAlarmRecord record : confirmed){
                String alarmId = record.getId();
                int adviceFlag = featureElementServiceImpl.getByAlarmId(alarmId).getDisposalAdvice();
                switch (adviceFlag) {
            ...
            //获取最新一条被确认为事件的告警记录
            OriginalAlarmRecord originalAlarmRecord = originalAlarmServiceImpl.getLatestUnconfirm(ids);//最新一条被确认为事件的记录
            if(alarmCollection.getLatestAlarmTime() == originalAlarmRecord.getAlarmTime()){
                return 3;
            }else{
                List<OriginalAlarmRecord> waitDealList = originalAlarmServiceImpl.getUnConfirmList(ids,originalAlarmRecord.getAlarmTime());//获取最新一条被确认为事件告警之后且未被确认为事件的告警记录
                //判断告警记录里面是否有
